...
Usage:
    python -m src.split   # creates data/processed/split.json
"""
...
def create_split():
    ...
    out_path = config.DATA_PROCESSED / "split.json"
...
def load_split():
    split_path = config.DATA_PROCESSED / "split.json"
    if not split_path.exists():
        raise FileNotFoundError(f"No split found at {split_path}. Run `python -m src.split` first.")
